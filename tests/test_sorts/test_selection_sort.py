...
class TestStringMethods(unittest.TestCase):
    ...
    def test_basic_int_array_selection_sort(self):
        array = self.basic_int_array
        length = len(array)
        result = self.result_for_basic_int_array
        self.assertEqual(selection_sort(array, length), result)

    def test_basic_float_array_selection_sort(self):
        array = self.basic_float_array
        length = len(array)
        result = self.result_for_basic_float_array
        self.assertEqual(selection_sort(array, length), result)

    def test_single_array_selection_sort(self):
        array = self.single_element_array
        length = len(array)
        result = self.result_for_single_element_array
        self.assertEqual(selection_sort(array, length), result)

    def test_empty_array_selection_sort(self):
        array = self.empty_array
        length = len(array)
        result = self.result_for_empty_array
        self.assertEqual(selection_sort(array, length), EMPTY_LIST)
